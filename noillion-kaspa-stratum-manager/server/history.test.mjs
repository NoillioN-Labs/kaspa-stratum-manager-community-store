...
test("persists sanitized seven-day samples, confirmed blocks, and probability estimates", async () => {
  ...
  const store = new MiningHistoryStore({
    path: historyPath,
    now: () => now,
    flushIntervalMs: SEVEN_DAYS_MS,
  });
    ...
    stats(102, [
      {
        ...
        wallet: "must-not-persist",
        hash: "block-a",
        timestamp: String((now - 60_000) / 1000),
      },
    ]),
  );
  await store.close();
  ...
  const reloaded = new MiningHistoryStore({
    path: historyPath,
    now: () => now,
  });
...
test("prunes samples and block events outside the seven-day window", async () => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-history-prune-"),
  );
  const historyPath = path.join(directory, "mining-history.json");
  ...
  await store.record({
    networkHashrate: 1e12,
    networkBlockCount: 1,
    workers: [{ worker: "old", hashrate: 1 }],
    blocks: [
      { worker: "old", hash: "old-block", timestamp: String(now / 1000) },
    ],
  });
  ...
  await store.record({
    networkHashrate: 1e12,
    networkBlockCount: 2,
    workers: [{ worker: "current", hashrate: 1 }],
    blocks: [],
  });
  ...
  assert.equal(summary.blockHistoryDays, null);
  assert.deepEqual(
    summary.workers.map(({ worker }) => worker),
    ["current"],
  );
});
...
test("calculates solo-mining performance windows, share freshness, quality, luck, effort, and charts", async () => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-history-performance-"),
  );
  const historyPath = path.join(directory, "mining-history.json");
  ...
  assert.equal(
    summary.workers[0].periods.oneHour.lastAcceptedShareAt,
...
test("atomically resets persisted mining history", async () => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-history-reset-"),
  );
  const historyPath = path.join(directory, "mining-history.json");
  let now = Date.parse("2026-08-30T00:00:00Z");
  const store = new MiningHistoryStore({
  ...
  await store.record({
    networkHashrate: 1e12,
    networkBlockCount: 10,
    workers: [{ worker: "RIG01", hashrateGhs: 1000 }],
  });
  now += 60_000;
  await store.record({
    networkHashrate: 1e12,
    networkBlockCount: 11,
    workers: [{ worker: "RIG01", hashrateGhs: 1000 }],
    blocks: [{ worker: "RIG01", hash: "block-reset-test" }],
  });
  await store.reset();
  ...
  assert.equal(summary.recentBlocks.length, 1);
  assert.equal(
    JSON.parse(await readFile(historyPath, "utf8")).blocks.length,
    1,
  );
});
...
test("migrates version-2 history to reward-aware version 3 without losing records", async () => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-history-migration-"),
  );
  const historyPath = path.join(directory, "mining-history.json");
      ...
      samples: [
        {
          timestamp: now,
          networkHashrate: 10,
          networkDifficulty: 20,
          networkBlockCount: 30,
          workers: [{ instance: "5555", worker: "RIG01", hashrateHs: 40 }],
        },
      ],
      blocks: [
        {
...
test("persists exact reward strings and aggregates realised revenue without precision loss", async () => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-history-reward-"),
  );
  const historyPath = path.join(directory, "mining-history.json");
  const now = Date.parse("2026-08-30T00:00:00Z");
  const store = new MiningHistoryStore({ path: historyPath, now: () => now });
  await store.record({
    ...
    blocks: [{ worker: "RIG01", hash: "b".repeat(64) }],
  });
  assert.equal(
    await store.updateBlockReward("b".repeat(64), {
      rewardStatus: "blue",
      blockColor: "blue",
