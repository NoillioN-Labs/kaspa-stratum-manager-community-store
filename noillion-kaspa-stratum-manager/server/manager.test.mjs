...
const testConfig = (directory) => ({
  ...
  listenPort: 0,
  profile: "test",
  nodeEndpoint: { host: "127.0.0.1", port: 1 },
  bridgeApiUrl: "http://127.0.0.1:1",
  bridgeCommand: "fake",
  bridgeArgs: [],
  ...
  settingsHealthTimeoutMs: 50,
  settingsHealthIntervalMs: 1,
  historyPath: path.join(directory, "mining-history.json"),
  historyRetentionMs: 7 * 24 * 60 * 60 * 1000,
  historySampleIntervalMs: 60_000,
  historyFlushIntervalMs: 300_000,
  metricsPath: path.join(directory, "dashboard-metrics.json"),
  metricsRetentionMs: 10 * 60 * 1000,
  metricsSampleIntervalMs: 5_000,
...
const putSettings = (port, body) =>
  fetch(`http://127.0.0.1:${port}/api/manager/settings`, {
    method: "PUT",
...
test("reports a reachable Umbrel node and bridge API", async (t) => {
  const node = net.createServer();
  const nodePort = await listen(node);
  const stratum = net.createServer();
  const stratumPort = await listen(stratum);
  const bridge = http.createServer((req, res) => {
    res.setHeader("content-type", "application/json");
    res.end(
      req.url === "/api/stats"
        ? JSON.stringify({ activeWorkers: 2 })
        : JSON.stringify({ kaspad_version: "2.0.1", instances: 1 }),
    );
  });
  ...
  const manager = createManager({
    ...
    bridgeVersion: "2.0.1",
    profile: "test",
    nodeEndpoint: { host: "127.0.0.1", port: nodePort },
    stratumEndpoint: { host: "127.0.0.1", port: stratumPort },
  ...
  t.after(async () => {
    await manager.close();
    await close(manager.server);
    await close(bridge);
    await close(stratum);
    await close(node);
  });
  const status = await fetch(
    `http://127.0.0.1:${managerPort}/api/manager/status`,
  ).then((r) => r.json());
  assert.equal(status.healthy, true);
  assert.equal(status.node.reachable, true);
  ...
  const stats = await fetch(
    `http://127.0.0.1:${managerPort}/api/manager/stats`,
  ).then((r) => r.json());
...
test("blocks process controls in an unmanaged Windows profile", async (t) => {
  ...
  const response = await fetch(
    `http://127.0.0.1:${port}/api/manager/bridge/start`,
    { method: "POST" },
  );
...
test("returns bounded in-memory manager and bridge logs", async (t) => {
  ...
  const response = await fetch(
    `http://127.0.0.1:${port}/api/manager/logs?limit=2`,
  );
  const body = await response.json();
  assert.equal(response.status, 200);
  assert.equal(body.lines.length, 2);
  assert.deepEqual(
    body.lines.map(({ source, line }) => ({ source, line })),
    [
  ...
  assert.ok(
    body.lines.every(({ timestamp }) => !Number.isNaN(Date.parse(timestamp))),
  );
});

test("returns only validated public donation addresses", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-manager-support-"),
  );
  ...
  assert.deepEqual(
    body.currencies.map(({ id }) => id),
    ["kaspa", "bitcoin"],
  );
  assert.deepEqual(Object.keys(body.currencies[0]).sort(), [
    "address",
    "id",
    "label",
  ]);
  assert.deepEqual(Object.keys(body.currencies[1]).sort(), [
    "address",
    "id",
    "label",
  ]);
...
test("hides malformed donation configuration", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-manager-support-invalid-"),
  );
  const config = testConfig(directory);
  ...
  const body = await fetch(`http://127.0.0.1:${port}/api/manager/support`).then(
...
test("returns sanitized durable mining history and block outlook", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-manager-history-"),
  );
  ...
  const first = {
    ...
    workers: [
      { instance: "5555", worker: "RIG01", wallet: "private", hashrate: 10 },
    ],
    blocks: [],
  };
    ...
    blocks: [
      {
        ...
        wallet: "private",
        hash: blockHash,
        timestamp: String((started + 60_000) / 1000),
      },
    ],
  };
...
test("resolves local blocks through Kaspad and exposes exact reward analytics", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-manager-rewards-"),
  );
  const config = testConfig(directory);
  ...
  payload.writeBigUInt64LE(90n, 8);
  payload[18] = 1;
  payload[19] = 0x51;
  let closed = false;
  const rewardRpc = {
    ...
    getBlockRewardInfo: async (hash) => ({
      hash,
      blockColor: "blue",
      confirmationCount: "12",
      mergingChainBlockHash: mergingHash,
      rewardAmountSompi: "9007199254740993",
    }),
    getBlock: async (hash) =>
      hash === blockHash
        ? {
            verboseData: { hash: blockHash },
            transactions: [{ payload: payload.toString("hex") }],
          }
        : {
            verboseData: {
              hash: mergingHash,
              mergeSetBluesHashes: [blockHash],
            },
            transactions: [
              {
                outputs: [
  ...
  await manager.history.record({
    workers: [{ worker: "RIG01", hashrate: 1 }],
    blocks: [{ worker: "RIG01", hash: blockHash }],
  });
  ...
  const health = await fetch(
    `http://127.0.0.1:${port}/api/manager/rewards/health`,
  ).then((response) => response.json());
  assert.equal(health.mode, "full");
  assert.equal(health.serverVersion, "2.0.1");
  const summary = await fetch(
    `http://127.0.0.1:${port}/api/manager/rewards/summary?period=7d`,
  ).then((response) => response.json());
  ...
  const blocks = await fetch(
    `http://127.0.0.1:${port}/api/manager/rewards/blocks`,
  ).then((response) => response.json());
...
test("requires confirmation and resets all retained miner statistics", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-manager-reset-"),
  );
  const manager = createManager(testConfig(directory), {
    supervisor: fakeSupervisor(),
  });
  ...
  await manager.history.record(
    {
      networkHashrate: 1e12,
      networkBlockCount: 100,
      workers: [{ worker: "RIG01", hashrateGhs: 1000 }],
    },
    started,
  );
  await manager.history.record(
    {
      ...
      networkBlockCount: 101,
      workers: [{ worker: "RIG01", hashrateGhs: 1000 }],
      blocks: [{ worker: "RIG01", hash: "block-reset" }],
    },
    started + 60_000,
  );
  await manager.metrics.record(
    { activeWorkers: 1, totalShares: 42, workers: [{ hashrateGhs: 1000 }] },
    started + 60_000,
  );
  const port = await listen(manager.server);
  t.after(async () => {
  ...
  const rejected = await fetch(
    `http://127.0.0.1:${port}/api/manager/statistics/reset`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({}),
  ...
  const response = await fetch(
    `http://127.0.0.1:${port}/api/manager/statistics/reset`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ confirmation: "DELETE MINER STATISTICS" }),
    },
  );
  assert.equal(response.status, 200);
  ...
  assert.equal(history.blocksFound, 1);
  assert.equal(history.recentBlocks.length, 1);
...
test("returns persistent rolling dashboard metrics without private worker data", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-manager-metrics-"),
  );
  const manager = createManager(testConfig(directory), {
    supervisor: fakeSupervisor(),
  });
...
test("reads only the sanitized settings model", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-settings-read-"),
  );
  ...
  assert.doesNotMatch(
    JSON.stringify(body),
    /host\.docker\.internal|kaspad_address|unrelated_supported_option/,
  );
});

test("rejects unknown fields, invalid ports, non-power-of-two difficulty, and incompatible tuning", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-settings-invalid-"),
  );
  await writeFile(path.join(directory, "config.yaml"), defaultYaml);
  let restarts = 0;
  const manager = createManager(testConfig(directory), {
    supervisor: fakeSupervisor(async () => {
      restarts += 1;
    }),
    waitForBridgeHealthy: async () => true,
  });
  ...
  const response = await putSettings(port, {
    ...AUTOMATIC_SETTINGS,
    preset: "custom",
    stratumPort: 80,
    minimumShareDifficulty: 3000,
    powerOfTwoClamp: false,
    wallet: "forbidden",
  });
  const body = await response.json();
  ...
  assert.deepEqual(
    new Set(body.issues.map(({ field }) => field)),
    new Set([
      "wallet",
      "stratumPort",
      "minimumShareDifficulty",
      "powerOfTwoClamp",
    ]),
  );
  assert.equal(restarts, 0);
  assert.equal(
    await readFile(path.join(directory, "config.yaml"), "utf8"),
    defaultYaml,
  );
});
...
test("atomically persists approved settings, preserves unrelated configuration, backs up, and restarts", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-settings-save-"),
  );
  await writeFile(path.join(directory, "config.yaml"), defaultYaml);
  let restarts = 0;
  const manager = createManager(testConfig(directory), {
  ...
  const response = await putSettings(port, settings);
  const body = await response.json();
  assert.equal(response.status, 200);
  assert.equal(body.result, "saved");
  assert.equal(body.bridgeRestarted, true);
  assert.equal(restarts, 1);
  assert.equal(
    await readFile(path.join(directory, "config.last-good.yaml"), "utf8"),
    defaultYaml,
  );
...
test("serializes concurrent settings updates", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-settings-concurrent-"),
  );
  await writeFile(path.join(directory, "config.yaml"), defaultYaml);
  let active = 0;
  let maximum = 0;
  ...
  const first = putSettings(port, {
    ...AUTOMATIC_SETTINGS,
    preset: "custom",
    sharesPerMinute: 20,
  });
  const second = putSettings(port, {
    ...AUTOMATIC_SETTINGS,
    preset: "custom",
    sharesPerMinute: 40,
  });
...
test("restores the last-known-good file and restarts after a failed new configuration", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-settings-rollback-"),
  );
  await writeFile(path.join(directory, "config.yaml"), defaultYaml);
  let restarts = 0;
  let checks = 0;
  const manager = createManager(testConfig(directory), {
    ...
    waitForBridgeHealthy: async () => {
      checks += 1;
      if (checks === 1) throw new Error("new bridge unhealthy");
      return true;
    },
  });
  const port = await listen(manager.server);
  ...
  assert.equal(body.result, "rolled_back");
  assert.deepEqual(body.rollback, {
    restored: true,
    restarted: true,
    healthy: true,
    error: null,
...
test("reports rollback recovery failure after restart failure", async (t) => {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "kaspa-settings-restart-failure-"),
  );
  await writeFile(path.join(directory, "config.yaml"), defaultYaml);
  let restarts = 0;
  const manager = createManager(testConfig(directory), {
    supervisor: fakeSupervisor(async () => {
      restarts += 1;
      throw new Error(
        restarts === 1 ? "restart failed" : "rollback restart failed",
      );
    }),
    waitForBridgeHealthy: async () => true,
  });
  ...
  assert.equal(response.status, 503);
  assert.equal(body.result, "rolled_back");
  assert.equal(body.rollback.restored, true);
  assert.equal(body.rollback.restarted, false);
  assert.equal(body.rollback.healthy, false);
...
test("asynchronous settings survives request completion, rejects duplicates and shares lifecycle queue", async (t) => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "ksm-operation-"));
  const config = testConfig(directory);
  await writeFile(config.settingsPath, defaultYaml);
  let release;
  ...
  const request = () =>
    fetch(`http://127.0.0.1:${port}/api/manager/settings?async=1`, {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(AUTOMATIC_SETTINGS),
    });
  ...
  const pending = await fetch(
    `http://127.0.0.1:${port}/api/manager/operations/active`,
  ).then((r) => r.json());
  assert.equal(pending.id, op.id);
  assert.equal(pending.state, "running");
  release();
  for (let i = 0; i < 30; i++) {
    const done = await fetch(
      `http://127.0.0.1:${port}/api/manager/operations/${op.id}`,
    ).then((r) => r.json());
...
test("cross-origin mutations rejected and concurrent readers share bridge collection", async (t) => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "ksm-collector-"));
  let calls = 0;
  const manager = createManager(testConfig(directory), {
    supervisor: fakeSupervisor(),
    fetchBridgeStats: async () => {
      calls++;
  ...
  assert.equal(
    (
      await fetch(`http://127.0.0.1:${port}/api/manager/bridge/restart`, {
        method: "POST",
  ...
  await Promise.all(
    Array.from({ length: 10 }, () =>
      fetch(`http://127.0.0.1:${port}/api/manager/stats`),
    ),
  );
