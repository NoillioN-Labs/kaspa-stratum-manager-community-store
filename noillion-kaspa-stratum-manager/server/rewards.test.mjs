...
test("decodes the little-endian uint64 subsidy without floating point", () => {
  ...
  assert.equal(
    decodeCoinbaseSubsidy(payloadFor("18446744073709551615")),
...
test("rejects malformed and truncated coinbase payloads", () => {
  assert.throws(
    () => decodeCoinbaseSubsidy("xyz"),
    (error) =>
      error instanceof RewardDecodeError && error.code === "INVALID_PAYLOAD",
  );
  assert.throws(
    () => decodeCoinbaseSubsidy("00".repeat(15)),
    (error) =>
      error instanceof RewardDecodeError && error.code === "SHORT_PAYLOAD",
  );
});
...
test("decomposes subsidy, fees, and DAG rewards by merge-set index", () => {
  ...
  const result = decomposeBlockReward({
    hash,
    mergingHash: "c".repeat(64),
    totalRewardSompi: "135",
    queriedBlock: {
      verboseData: { hash },
      transactions: [{ payload: payloadFor(100) }],
    },
    mergingBlock: {
      verboseData: {
        hash: "c".repeat(64),
        mergeSetBluesHashes: ["b".repeat(64), hash],
      },
      transactions: [
        {
          outputs: [
  ...
  assert.deepEqual(result, {
    subsidySompi: "100",
    acceptedTxFeesSompi: "20",
    dagMergeRewardSompi: "15",
    totalRewardSompi: "135",
  ...
  assert.throws(
    () =>
      decomposeBlockReward({
        ...
        mergingBlock: {
          verboseData: { hash: "c".repeat(64), mergeSetBluesHashes: [hash] },
          transactions: [
            {
              outputs: [
                {
  ...
  assert.throws(
    () =>
      decomposeBlockReward({
        hash,
        mergingHash: "c".repeat(64),
        totalRewardSompi: "120",
        queriedBlock: {
          verboseData: { hash },
          transactions: [{ payload: payloadFor(100) }],
        },
        mergingBlock: {
          verboseData: { hash: "c".repeat(64), mergeSetBluesHashes: [] },
          transactions: [
            {
              outputs: [
                {
