...
const protoPath = fileURLToPath(
  new URL("./proto/kaspa-v2.0.1-minimal.proto", import.meta.url),
);
...
export const normalizeBlockHash = (value) => {
  ...
  if (!HASH_PATTERN.test(hash))
    throw new KaspaRpcError(
      "INVALID_BLOCK_HASH",
      "Block hash must contain exactly 64 hexadecimal characters.",
    );
  return hash;
};
...
export class KaspaRpcAdapter extends EventEmitter {
  constructor({
    endpoint,
    ...
    if (!endpoint || typeof endpoint !== "string")
      throw new KaspaRpcError(
        "INVALID_ENDPOINT",
        "A local Kaspad gRPC endpoint is required.",
      );
    this.endpoint = endpoint;
    this.timeoutMs = Math.max(250, Number(timeoutMs) || DEFAULT_TIMEOUT_MS);
    this.clientFactory =
      clientFactory ?? (() => new rpcClient(this.endpoint, credentials));
    this.client = null;
    this.stream = null;
  ...
  connect() {
    ...
    this.stream.on("data", (message) => {
      if (current === this.stream) this.receive(message);
    });
    this.stream.on(
      "error",
      (error) =>
        ...
        this.disconnectStream(
          new KaspaRpcError(
            "TRANSPORT_ERROR",
            `Kaspad RPC stream failed: ${error.message}`,
            error,
          ),
        ),
    );
    this.stream.on(
      ...
      () =>
        current === this.stream &&
        this.disconnectStream(
          new KaspaRpcError("TRANSPORT_CLOSED", "Kaspad RPC stream closed."),
        ),
    );
  }

  receive(message) {
    if (message.blockAddedNotification)
      this.emit("block", message.blockAddedNotification.block);
    if (message.virtualChainChangedNotification)
      this.emit("chain", message.virtualChainChangedNotification);
    ...
    if (!response)
      return pending.reject(
        new KaspaRpcError(
          "UNEXPECTED_RESPONSE",
          `Kaspad returned an unexpected response for request ${id}.`,
        ),
      );
    if (response.error?.message)
      return pending.reject(
        new KaspaRpcError("RPC_ERROR", response.error.message),
      );
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(
          new KaspaRpcError(
            "TIMEOUT",
            `Kaspad did not answer within ${this.timeoutMs} ms.`,
          ),
        );
      }, this.timeoutMs);
      ...
      this.stream.write({ id, [requestName]: payload }, (error) => {
        ...
        reject(
          new KaspaRpcError(
            "TRANSPORT_ERROR",
            `Kaspad RPC request failed: ${error.message}`,
            error,
          ),
        );
      });
  ...
  async getBlockRewardInfo(value) {
    ...
    return {
      ...
      mergingChainBlockHash:
        response.mergingChainBlockHash?.toLowerCase() ?? null,
  ...
  async getBlock(value, includeTransactions = true) {
    const hash = normalizeBlockHash(value);
    const response = await this.call("getBlockRequest", "getBlockResponse", {
      hash,
      includeTransactions,
    });
  ...
  close() {
    this.disconnectStream(
      new KaspaRpcError("CLIENT_CLOSED", "Kaspad RPC client closed."),
    );
    this.client?.close?.();
    this.client = null;
  }
