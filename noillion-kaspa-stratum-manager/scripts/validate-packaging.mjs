...
assert.match(manifest, /^id: noillion-kaspa-stratum-manager$/m);
assert.match(manifest, /^category: crypto$/m);
assert.match(manifest, /^icon: https:\/\/raw\.githubusercontent\.com\/NoillioN-Labs\/umbrel-community-app-store\/main\/noillion-kaspa-stratum-manager\/icon\.svg$/m);
assert.match(compose, /APP_HOST: noillion-kaspa-stratum-manager_web_1/);
...
assert.match(manager, /\/api\/manager\/support/);
assert.match(manager, /KSM_DONATION_KASPA_ADDRESS/);
assert.match(manager, /KSM_DONATION_BITCOIN_ADDRESS/);
